...
from aggregative_vi import agg_vi


class aggregative_hypergradient(distributed_hg.distributed_hypergradient):
    def __init__(
        self,
        Q,
        Qreg,
        ...
        # (eye_N \kron (Q + Q_reg) + ones_{N x N} \kron Q) * y + ones_N \kron C * x + ones_N \kron c
        ...
        if step_guess is None:
            Q_all = np.kron(
                np.eye(self.n_ag) + np.ones((self.n_ag, self.n_ag)), self.Q
            ) + np.kron(np.eye(self.n_ag), Qreg)
            # Lipschitz constant
            self.lf = np.sqrt(
                scipy.linalg.eigh(
    ...
    def run_fixed_agg_lead(self, inner_iters, n_iters=1, timing=False):
    ...
    def save_total_time(self, path, n_ag, tsteps):
        # Followers' time
